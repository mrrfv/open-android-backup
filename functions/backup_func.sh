...
function backup_func() {
  ...
  for app in $(adb shell pm list packages -3 -f); do
    ...
    apk_clean_name=$(echo "$app" | awk -F "=" '{print $NF}' | tr -dc '[:alnum:].' | tr '[:upper:]' '[:lower:]')
    apk_base="$apk_clean_name-$RANDOM$RANDOM.apk"

    # Extract the base APK
    adb pull "$apk_path" "backup-tmp/Apps/$apk_base"

    # Extract split APKs
    split_apks=$(get_split_apks "$apk_clean_name")
    index=0
    for split_apk_path in $split_apks; do
      split_apk_base="${apk_clean_name}_${index}_$(basename "$split_apk_path")"
      adb pull "$split_apk_path" "backup-tmp/Apps/$split_apk_base"
      index=$((index+1))
    done
  done
